...
function LoginCallback() {
  ...
  const { loadWatchedStock } = useContext(StockContext);

  useEffect(() => {
    // Function to handle token extraction and validation
    const handleLoginCallback = async () => {
      try {
        ...
        if (response.data.success) signin(userObj);
        else setError("Cannot find authorized user information");
        if (userObj.watchedStocks) loadWatchedStock(token);
